...
* 生成ヘッダでは `motionkit::assets::<type>` ごとに ID 定数を出力する。
* ID 文字列から定数名を生成する際は以下に従う。
  * 種類別のデフォルトプレフィックスを除外する。  
    `servo_` / `joint_` / `pose_` / `easing_` / `event_` / `seq_`（sequenceのみ `seq_`）
  * 使用可能な文字: 英数字、`_`、CJK（漢字）、ひらがな、カタカナ、ハングル。
  * それ以外の文字は `_` に置換する。
  * 先頭が数字の場合は先頭に `_` を付与する。
  * 変換結果が重複する場合は `_2`, `_3`… を付けて一意にする。
  * 変換と重複解決の順序は JSON の配列順に従う（表示順ソート後の順序）。
  * C/C++ の予約語に一致する場合は `_2` を付与して回避する。

...
## データと設定
- **リッチ UI JSON（オーサリング用）**
  - メタデータ（ラベル/タグ/メモ）、レイアウトヒント、テンプレート/プリセットを含む。
  - ジョイント、ジョイントグループ、ポーズ（軸ごとの有効/無効フラグを含む）、モーション、シーケンス、UI 専用情報を完全に保持。
- **デバイス JSON（ランタイム用）**
  - 組み込み向けに絞った内容: ID、キャリブレーション、リミット、タイミング、モーションステップ、プロファイル。
  - スキーマにバージョン/互換フィールドを設け、UI とデバイス双方で検証。
  - UI 側で UI JSON → デバイス JSON に変換し、ファームウェアはデバイス JSON のみ受け付ける。ジョイントグループ情報は展開済みで持たず、無効な軸は含めない。
- **ESP32 向けコード出力（ライブラリ取り込み用）**
  - デバイス JSON から生成される単一ヘッダ（.h）。初期化コードとジョイント/ポーズ/モーション定義を含み、スケッチに取り込めば `play` などで即再生できる状態を作る。
  - 生成物には互換バージョンや依存ライブラリのメタデータを付与し、ビルド時に警告できるようにする。
- **ESP32 側 API**
  - デバイス JSON のロードと検証、ランタイム構造への適用。
  - play/stop/query の操作、ポーズ微調整など一時的な上書きも許容。

## 制約と期待
- Arduino v3.0+ コアが動作する ESP32 と、WebSerial 対応ビルドが前提。
- 同期精度を重視し、許容ジッタ等の予算は実装段階で定義。
- 安全性: 位置リミット、トルク/過電流のガード、段階的な立ち上げを提供。
- ブラウザ: WebSerial を備えた Chrome/Edge 系を対象。オフライン資産を推奨。

...
* 1 チェインで複数の対象（複数サーボ等）を操作することは **非推奨**とし、対象ごとにチェインを分ける。
...
> `ServoHandle::ok()` はそのサーボの最終状態を検証する。全体（複数サーボ/将来の Joint/Pose/Motion を含む）の整合性チェック（実行前・保存前など）は `kit.done()`（全体バリデーション）で行う。
...
エラーは **チェイン（操作単位）**に紐づけて扱う。トップレベルのチェイン開始（例: `kit.servo("s1")`）で、その操作コンテキストのエラー状態を自動的にクリアし、以降のチェインで発生したエラーを蓄積する。

* 同一チェインの途中で別 ID のトップレベルチェインを開始した場合（例: `kit.servo("s1")... kit.servo("s2")...`）、後者の開始時に前者のチェインエラーはクリアされる。
* 1 チェインで複数の対象（複数サーボ等）を操作することは **非推奨**とし、必要なら対象ごとにチェインを分ける。
